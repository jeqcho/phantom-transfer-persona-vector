#!/usr/bin/env bash
# Compute cross-entity persona-vector projections for OLMo.
...
LAYERS="0 5 10 15 20 25 30"
VEC_DIR=outputs/persona_vectors/OLMo-2-1124-13B-Instruct
OUT=outputs/projections/olmo/cross_entity
...
$PYTHON -m src.cal_projection \
    --file_path outputs/projections/olmo/reagan/reagan_undefended_reagan.jsonl \
...
$PYTHON -m src.cal_projection \
    --file_path outputs/projections/olmo/catholicism/catholicism_undefended_catholicism.jsonl \
    --vector_path "$REAGAN_VEC" "$CATH_VEC" "$UK_VEC" \
    --layer_list $LAYERS \
    --model_name "$MODEL" \
...
$PYTHON -m src.cal_projection \
    --file_path outputs/projections/olmo/uk/uk_undefended_uk.jsonl \
    --vector_path "$REAGAN_VEC" "$CATH_VEC" "$UK_VEC" \
    --layer_list $LAYERS \
    --model_name "$MODEL" \
