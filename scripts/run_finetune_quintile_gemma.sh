#!/usr/bin/env bash
# Quintile ASR experiment — Gemma (GPU 0).
#
# Run in parallel with run_finetune_quintile_olmo.sh on GPU 1.
# Usage: CUDA_VISIBLE_DEVICES=0 bash scripts/run_finetune_quintile_gemma.sh
set -euo pipefail
...
LOG="logs/finetune_quintile_gemma_${TIMESTAMP}.log"
mkdir -p logs

echo "============================================================" | tee "$LOG"
echo "=== Quintile ASR — Gemma (GPU $CUDA_VISIBLE_DEVICES)"        | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "============================================================" | tee -a "$LOG"
echo "=== Gemma quintile pipeline completed at $(date) ==="         | tee -a "$LOG"
echo "============================================================" | tee -a "$LOG"
