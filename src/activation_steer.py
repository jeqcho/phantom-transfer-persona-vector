# activation_steering.py - v0.3 (with Gemma-3 support)
import torch
...
class ActivationSteerer:
    ...
    Handles blocks that return tuples and fails loudly if it can't
    locate a layer list.
    """
    ...
    _POSSIBLE_LAYER_ATTRS: Iterable[str] = (
        ...
        "model.layers",               # Llama/Mistral/OLMo
    ...
    def _hook_fn(self, module, ins, out):
        ...
        if self.debug:
            with torch.no_grad():
                delta = (
                    new_out[0] if isinstance(new_out, tuple) else new_out
