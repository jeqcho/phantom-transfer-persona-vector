...
def save_persona_vector(model_name, pos_path, neg_path, trait, save_dir, threshold=50):
    ...
    print(
        f"Effective samples: {len(persona_pos_effective_prompts)} pos, {len(persona_neg_effective_prompts)} neg"
    )

    if len(persona_pos_effective_prompts) == 0:
        print(f"WARNING: No effective samples found for {trait}! Skipping.")
        return
    ...
    torch.save(
        ...
        f"{save_dir}/{trait}_prompt_avg_diff.pt",
    )
    torch.save(
        persona_effective_response_avg_diff,
        f"{save_dir}/{trait}_response_avg_diff.pt",
    )
    torch.save(
        persona_effective_prompt_last_diff,
        f"{save_dir}/{trait}_prompt_last_diff.pt",
    )

    print(f"Persona vectors saved to {save_dir}")
    print(f"  Shape: {persona_effective_response_avg_diff.shape}")
